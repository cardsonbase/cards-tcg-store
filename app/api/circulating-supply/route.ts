...
const LOCKED_ADDRESSES = [
  '0x10B5F02956d242aB770605D59B7D27E51E45774C',  // Replace with actual addresses
  '0x4B52d5C253b7e668a1FB1780C6EF282ACEAEeaa4',
  // Add marketing wallet after liquidity focused
];
...
export async function GET() {
  ...
  const total = 1000000000000000000n; // 1B with 9 decimals
  const existing = total - burned;
  const circulating = existing - locked;

  const circulatingStr = (Number(circulating) / 1e9).toFixed(9);

  return new NextResponse(circulatingStr, {
